...
package SimplePumpingSystem
  ...
  model CheckPumpingSystem "Check model PumpingSystem"
    import
      .Requirements.Examples.SimplePumpingSystem.Components.Bindings;
    extends
      .Requirements.Examples.SimplePumpingSystem.Components.PumpingSystem;
    ...
    annotation(experiment(StopTime=2000, Tolerance=1e-006),
      Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{160,
              100}})),
      Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{100,100}}),graphics = {Ellipse(origin={124,38},extent={{-81.87294480257326,78.34578294866141},{82,-78}},fillPattern=FillPattern.Solid,fillColor={255,0,0})}),Documentation(info = "<html><p>The PumpRequirements uses illegal Modelica for very stupid reasons, to be able to pass classes ( instead of records) into a function, NOT legal Modelica. It would be no issue if they had not reused an example from MSL that they couldn't edit. It would be simple if the observation records would be added to the observed components instead, e.g. a record of type Records.Pump with instacne name pump to the pump class in PumpingSystem. A lot of work to fix, for zero gain!</p></html>"));
...
<li><i>2 Nov 2005</i>
    ...
    package Requirements "Observation records"
      ...
      block TankRequirements "Requirements for a Tank"
        extends .Requirements.Interfaces.PartialRequirements;
        ...
        .Requirements.Verify.BooleanRequirement R_InBand(text="Tank level must be within given bounds")
          annotation (Placement(transformation(extent={{20,50},{80,70}})));
        .Requirements.Verify.BooleanRequirement R_LimLevel(text="Tank level cannot be below " + String(limLevel) +
      ...
      block PumpRequirements "Requirements for one pump"
        ...
        .Requirements.Verify.BooleanRequirement R_speedMax(text="Pump speed is limited by " + String(max_rpm) + " rpm")
          annotation (Placement(transformation(extent={{20,50},{80,70}})));
        .Requirements.Verify.BooleanRequirement R_cavPortA(text="Pump must not cavitate at port_a")
          annotation (Placement(transformation(extent={{20,20},{80,40}})));
        .Requirements.Verify.BooleanRequirement R_cavPortB(text="Pump must not cavitate at port_b")
      ...
      equation
        ...
        annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
                  -100},{100,100}}), graphics), Icon(coordinateSystem(
                preserveAspectRatio=false, extent={{-100,-100},{100,100}}), graphics={
                Bitmap(extent={{-18,50},{90,-52}}, fileName="modelica://Requirements/Resources/Images/Examples/PumpingSystem/pump.png")}));
      ...
      block SourceRequirements "Requirements for a Source component"
        ...
        .Requirements.Verify.BooleanRequirement R_source[size(observation.p,1)](
